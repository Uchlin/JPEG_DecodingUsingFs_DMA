...
static void ExtractMazeArray(uint32_t xPos, uint32_t yPos, JPEG_ConfTypeDef *jpegInfo)
{
    ...
    for(y = 0; y < MAZE_HEIGHT; y++)
    {
        for(x = 0; x < MAZE_WIDTH; x++)
        {
            ...
            uint8_t avgBrightness = sumBrightness / (scale * scale);
...
static void drawPathGreen(uint16_t startX, uint16_t startY, uint16_t endX, uint16_t endY)
{
    uint32_t *pFrameBuffer = (uint32_t *)LCD_FRAME_BUFFER;
    uint8_t scale = 6;
    uint8_t markerExtra = 6;

    uint16_t displayXPos = (LCD_X_Size - MAZE_WIDTH * scale) / 2;
    uint16_t displayYPos = (LCD_Y_Size - MAZE_HEIGHT * scale) / 2;

    uint16_t currentX = endX;
    uint16_t currentY = endY;
    ...
    while (currentX != startX || currentY != startY)
    {
        for (uint16_t dy = 0; dy < scale; dy++)
        {
            for (uint16_t dx = 0; dx < scale; dx++)
            {
                pFrameBuffer[(displayYPos + currentY * scale + dy) * LCD_X_Size +
                             (displayXPos + currentX * scale + dx)] = 0xFF00FF00;
            }
        }

        uint16_t packed = prev[currentY][currentX];
        if (packed == 0xFFFF) break;

        currentX = packed & 0xFF;
        currentY = (packed >> 8) & 0xFF;

        if (currentX >= MAZE_WIDTH || currentY >= MAZE_HEIGHT) break;
    }
    void drawMarker(uint16_t mx, uint16_t my, uint32_t color)
    {
        for (int16_t dy = -markerExtra; dy < scale + markerExtra; dy++)
        {
            for (int16_t dx = -markerExtra; dx < scale + markerExtra; dx++)
            {
                int16_t cellX = mx + (dx >= 0 && dx < scale ? 0 : (dx < 0 ? -1 : 1));
                int16_t cellY = my + (dy >= 0 && dy < scale ? 0 : (dy < 0 ? -1 : 1));

                if (cellX < 0 || cellX >= MAZE_WIDTH || cellY < 0 || cellY >= MAZE_HEIGHT)
                    continue;
                if (mazeArray[cellY][cellX] == 1)
                {
                    int32_t posY = displayYPos + my * scale + dy;
                    int32_t posX = displayXPos + mx * scale + dx;

                    if (posY >= 0 && posY < LCD_Y_Size && posX >= 0 && posX < LCD_X_Size)
                    {
                        pFrameBuffer[posY * LCD_X_Size + posX] = color;
                    }
                }
            }
        }
    }
    drawMarker(startX, startY, 0xFF0000FF);
    drawMarker(endX, endY, 0xFFFF0000);
...
static void solveMaze(void)
{
    ...
    if (found)
    {
        drawPathGreen(startX, startY, endX, endY);
    }
}
int main(void)
{
  ...
  if(FATFS_LinkDriver(&SD_Driver, SDPath) == 0)
  {
    ...
    if(BSP_SD_IsDetected(0))
    {
      ...
      if(f_mount(&SDFatFs, (TCHAR const*)SDPath, 0) == FR_OK)
      {
        ...
        if(f_open(&JPEG_File, "image1.jpg", FA_READ) == FR_OK)
        {
...
//          ChangePixelColor(xPos, yPos, 0xFF00FF00);
//          AnalyzeImageBrightness(xPos, yPos, &JPEG_Info);
//          ExtractMazeArray(xPos, yPos, &JPEG_Info);
//          HAL_Delay(2000);
//          DisplayMazeFromArray();
          ExtractMazeArray(xPos, yPos, &JPEG_Info);
          HAL_Delay(2000);
          DisplayMazeFromArray();
          solveMaze();
