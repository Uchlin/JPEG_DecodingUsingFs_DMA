...
static void OpenAndDisplayJpeg(uint8_t idx)
{
    ...
    UTIL_LCD_FillRect(0, 0, 160, LCD_Y_Size, UTIL_LCD_COLOR_LIGHTBLUE);
    UTIL_LCD_DisplayStringAt(40, 210, (uint8_t*)"LEFT", LEFT_MODE);
    UTIL_LCD_DisplayStringAt(40, 240, (uint8_t*)"Back", LEFT_MODE);
    UTIL_LCD_FillRect(LCD_X_Size - 160, 0, 160, LCD_Y_Size, UTIL_LCD_COLOR_LIGHTBLUE);
...
static void DisplayFileMenu(void)
{
    ...
    UTIL_LCD_FillRect(0, 0, LCD_X_Size, 40, UTIL_LCD_COLOR_LIGHTBLUE);
    ...
    for (uint8_t i = 0; i < maxLines; i++) {
        ...
        if (fileIndex < fileCount && fileIndex == selectedFile) {
        	UTIL_LCD_SetBackColor(UTIL_LCD_COLOR_LIGHTBLUE);
            UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_WHITE);
            UTIL_LCD_FillRect(0, yPos - 2, LCD_X_Size, 28, UTIL_LCD_COLOR_LIGHTBLUE);
    ...
    UTIL_LCD_SetBackColor(UTIL_LCD_COLOR_LIGHTBLUE);
    UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_WHITE);
    UTIL_LCD_FillRect(0, LCD_Y_Size - 45, LCD_X_Size, 45, UTIL_LCD_COLOR_LIGHTBLUE);
    UTIL_LCD_SetFont(&Font24);

    char instructions[50];
    if (fileCount > 0) {
        sprintf(instructions, "UP/DOWN: Select   RIGHT: Open");
    } else {
